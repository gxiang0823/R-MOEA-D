...
for example_count = 1:20
    X = Example_10{example_count};
    P = Refer_10{example_count};
    Cusnum_Number = size(X,1)-1;            %Number of Target Points
    Cap = 50;                               %Vehicle Loading Capacity                       %-----10-----50-----5
    Vehicle_Number = 5;                     %Number of Vehicle                              %-----20-----50-----10
...
%     Pop = Random_Init(Cusnum_Number,Vehicle_Number,NIND,Rule_Number,Target_Demand,Cap);
    ...
    IGD = mean(IGDAll);
    HV = mean(HVAll);
    IGD_10(example_count) = IGD;
    HV_10(example_count) = HV;
end
