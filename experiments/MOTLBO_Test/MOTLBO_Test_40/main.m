...
for example_count = 1:20
    X = Example_40{example_count};
    P = Refer_40{example_count};
    Cusnum_Number = size(X,1)-1;            %Number of Target Points
    Cap = 100;                              %Vehicle Loading Capacity                       %-----10-----50-----5
...
%     Pop = Random_Init(Cusnum_Number,Vehicle_Number,NIND,Rule_Number,Target_Demand,Cap);
    ...
    IGD = mean(IGDAll);
    HV = mean(HVAll);
    IGD_40(example_count) = IGD;
    HV_40(example_count) = HV;
end
save('IGD_40.mat','IGD_40');
save('HV_40.mat','HV_40');