function Chrom_Decoded = Decoding_3(Route,Cusnum_Number,Target_Demand,Cap,Dist_Table)
...
while i <= length(Route)
    ...
    elseif Load_Sum == Cap
        ...
        if i ~= length(Route)
            if Dist_Table(Route(i)+1,1) <= Dist_Table(Route(i+1)+1,1)
                Path = Route(i);
                Load = Load_Sum;
                i = i + 1;
            else
                Path = [Route(i+1), Route(i)];
                Load = [Target_Demand(Route(i+1)), Load_Sum];
                Load_Sum = Load_Sum + Target_Demand(Route(i+1));
                i = i + 2;
            end
        else
            Path = Route(i);
