function Chrom_Decoded = Decoding_5(Route,Cusnum_Number,Target_Demand,Cap)
...
for i = 1:length(Route)
    ...
    elseif Load_Sum == Cap
        ...
        if Target_Demand(Route(i)) > mean(Load)
            Temp_Path{Vehicle_Count} = Path;
            Temp_Load{Vehicle_Count} = Load;
            Vehicle_Count = Vehicle_Count+1;
            Load_Sum = Target_Demand(Route(i));
            Path = Route(i);
            Load = Target_Demand(Route(i));
        else
            [maxval,maxidx] = max(Load);
            Temp_P = Path(maxidx);
            Temp_L = maxval;
            Path(maxidx) = Route(i);
            Load(maxidx) = Target_Demand(Route(i));
            Temp_p = [Path, Temp_P];
            Path = Temp_p;
            Temp_l = [Load, Temp_L - (Load_Sum - Cap)];
            Load = Temp_l;
            Temp_Path{Vehicle_Count} = Path;
            Temp_Load{Vehicle_Count} = Load;
            Vehicle_Count = Vehicle_Count+1;
